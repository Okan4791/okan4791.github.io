...
try {
  ...
  const motion = await send('Runtime.evaluate', { expression: `(()=>{
    ...
    return {packetTravel:Math.hypot(packetEnd.x-packetStart.x,packetEnd.y-packetStart.y),discTravel:Math.hypot(discMiddle.x-discStart.x,discMiddle.y-discStart.y),discProgress:discEnd.x>discMiddle.x,landingError:Math.hypot(discEnd.x-basket.x,discEnd.y-basket.y),discOpacity,ufoOpacity,beamOpacity,tapOpacity:Number(getComputedStyle(tapForm).opacity),tapEntry,tapFilled,tapExit,liquidBeforeFill,liquidScale:getComputedStyle(liquid).scale,foamBeforeFill,foamDuringFill,bigfootTravel:Math.hypot(bigfootBike.x-bigfootCamp.x,bigfootBike.y-bigfootCamp.y),bigfootLift:bigfootCamp.y-bigfootLifted.y,bigfootDrop:bigfootBike.y-bigfootLifted.y,beamAttachment:Math.hypot(bigfootBeam.x-beamCenter.x,bigfootBeam.y-beamCenter.y),bikeAttachment:Math.hypot(bigfootBike.x-bikeCenter.x,bigfootBike.y-bikeCenter.y),riderSaddleError,ufoBikeClearance,rideDirection,bigfootPoints:{camp:bigfootCamp,beam:bigfootBeam,lifted:bigfootLifted,bike:bigfootBike,beamCenter,bikeCenter,riderHip,saddle,rideStart,rideEnd},bigfootOpacity,throwerExit,companions,armStart,armEnd,wheelStart,wheelEnd,frame:getComputedStyle(frame).transform,discSize:dimensions(discForm),ufoShell:dimensions(document.querySelector('.ufo-shell')),ufoBeam:dimensions(document.querySelector('.ufo-beam')),tap:dimensions(tapForm),labels};
  })()`, returnByValue: true });
  const motionValue = motion.result.value;
  if (captureEvidence) console.log(`SVG motion evidence: ${JSON.stringify(motionValue)}`);
  if (motionValue.packetTravel < 10 || motionValue.discTravel < 100 || !motionValue.discProgress || motionValue.landingError > 20 || motionValue.discOpacity > .1 || motionValue.ufoOpacity < .9 || motionValue.ufoShell.width > motionValue.discSize.width * 1.5 || motionValue.beamOpacity[0] > .05 || motionValue.beamOpacity[1] < .8 || motionValue.beamOpacity[2] > .05 || motionValue.beamOpacity[3] < .8 || motionValue.beamOpacity[4] > .05 || motionValue.tapOpacity < .9 || motionValue.tapEntry.opacity > .3 || motionValue.tapEntry.scale === 'none' || motionValue.tapFilled.opacity < .9 || motionValue.tapFilled.scale !== '1' || motionValue.tapExit.opacity > .6 || motionValue.tapExit.scale === '1' || motionValue.liquidBeforeFill > .01 || motionValue.liquidScale === 'none' || motionValue.liquidScale === '1' || motionValue.foamBeforeFill > .01 || motionValue.foamDuringFill < .9 || motionValue.bigfootTravel < 100 || motionValue.bigfootLift < 5 || motionValue.bigfootDrop < 5 || motionValue.beamAttachment > 30 || motionValue.bikeAttachment > 35 || motionValue.riderSaddleError > 12 || motionValue.ufoBikeClearance < 1 || motionValue.rideDirection < 50 || motionValue.bigfootOpacity[0] < .9 || motionValue.bigfootOpacity[1] > .05 || motionValue.bigfootOpacity[2] < .9 || motionValue.throwerExit.opacity > .8 || motionValue.throwerExit.translate === 'none' || motionValue.companions.girlOpacity < .9 || motionValue.companions.dogOpacity < .9 || motionValue.companions.dogBody === motionValue.companions.dogHead || motionValue.companions.glasses === motionValue.companions.dogBody || motionValue.armStart === motionValue.armEnd || motionValue.wheelStart === motionValue.wheelEnd || motionValue.frame !== 'none' || motionValue.labels.join('|') !== 'Hiking|Camping|Cycling|Craft beer|Disc golf') throw new Error(`SVG motion regression: ${JSON.stringify(motionValue)}`);
