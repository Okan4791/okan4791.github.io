...
try {
  ...
  const motion = await send('Runtime.evaluate', { expression: `(()=>{
    ...
    const beam=document.querySelector('.ufo-beam');const beamAnimation=beam.getAnimations()[0];beamAnimation.pause();const beamOpacity=[];for(const time of [2400,4800,6000,8160,9600]){beamAnimation.currentTime=time;beamOpacity.push(Number(getComputedStyle(beam).opacity))}
    ...
    const anatomy=document.querySelector('.bigfoot-anatomy');const posture=anatomy.getAnimations()[0];posture.pause();posture.currentTime=9120;hobby.setCurrentTime(9.12);const screenPoint=(element,x,y)=>{const point=new DOMPoint(x,y).matrixTransform(element.getScreenCTM());return {x:point.x,y:point.y}};const riderHip=screenPoint(anatomy,0,15);const saddle=screenPoint(document.querySelector('.route-bike'),-6,28);const riderSaddleError=Math.hypot(riderHip.x-saddle.x,riderHip.y-saddle.y);const ufoBox=document.querySelector('.ufo-shell').getBoundingClientRect();const bike=document.querySelector('.route-bike');const bikeBox=bike.getBoundingClientRect();const rideStart=center(bikeBox);hobby.setCurrentTime(11.76);const rideEnd=center(bike.getBoundingClientRect());const rideDirection=rideEnd.x-rideStart.x;const ufoBikeClearance=bikeBox.top-ufoBox.bottom;
    ...
    const flightPath=document.querySelector('#hobby-flight-path');const fixedInFlight=new DOMPoint(fixedDiscAtTap.x,fixedDiscAtTap.y).matrixTransform(hobby.getScreenCTM().inverse());const pathSamples=[.745,.755,.765,.775,.785].map((progress)=>{const point=flightPath.getPointAtLength(flightPath.getTotalLength()*progress).matrixTransform(flightPath.getScreenCTM());return {targetX:fixedInFlight.x,targetY:fixedInFlight.y,progress,x:point.x,y:point.y,gap:Math.hypot(point.x-fixedDiscAtTap.x,point.y-fixedDiscAtTap.y)}});
  ...
  if (treeVisibility[0] < .9 || treeVisibility[1] < .9 || treeVisibility[2] < .9 || treeVisibility[3] > .1) throw new Error(`SVG camp tree timing: ${JSON.stringify(treeVisibility)}`);
  if (details.throwing > .1 || details.relaxed < .9 || details.disc === details.rim || details.canopy === details.hull || details.dogPaths !== 2) throw new Error(`SVG detail regression: ${JSON.stringify(details)}`);
