...
try {
  ...
  await send('Page.navigate', { url: 'http://127.0.0.1:4173/' });
  await new Promise((done) => setTimeout(done, 200));
  const commandPalette = await send('Runtime.evaluate', { expression: `(()=>{const picker=document.querySelector('.palette-picker');document.dispatchEvent(new KeyboardEvent('keydown',{key:'p',altKey:true,bubbles:true}));const pickerOpened=!picker.hidden&&picker.open;document.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}));const pickerClosed=picker.hidden&&!picker.open;document.dispatchEvent(new KeyboardEvent('keydown',{key:'k',altKey:true,bubbles:true}));const dialog=document.querySelector('.command-palette');const input=dialog.querySelector('input');input.value='privacy';input.dispatchEvent(new Event('input',{bubbles:true}));const selected=dialog.querySelector('[aria-selected="true"]');const state={open:dialog.open,focused:document.activeElement===input,results:dialog.querySelectorAll('[role="option"]').length,selected:selected?.querySelector('b')?.textContent,pickerHidden:picker.hidden,pickerOpened,pickerClosed};dialog.close();return state})()`, returnByValue: true });
  const commandValue = commandPalette.result.value;
  if (!commandValue.open || !commandValue.focused || commandValue.results < 1 || commandValue.selected !== 'Privacy Devices' || !commandValue.pickerHidden || !commandValue.pickerOpened || !commandValue.pickerClosed) throw new Error(`command palette failed: ${JSON.stringify(commandValue)}`);
